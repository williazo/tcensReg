...
```{r univariate_cens_plot, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center',fig.height=4, fig.width=6,tidy=FALSE}
...
```{r univariate_uncens_plot, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center',fig.height=4, fig.width=6,tidy=FALSE}
...
By default the coefficients are returned along with log likelihood and other fit criterion statistics. Note that the Psuedo R2 in the case of an intercept only model is exactly equal to zero.

```{r t_mod_full_print}
names(t_mod)
```
Note that the this object contains parameter estimates theta, convergence criterion code, initial/final log-liklihood values, variance-covariance matrix, method of optimization, information criterion and the design matrix used from the model.

Comparing the values to the truth we see that the estimates are unbiased.
```{r t_mod_est_compare, message=FALSE}
#tcensReg model
...
```{r speed comparison, message=FALSE, warning=FALSE, cache=TRUE}
